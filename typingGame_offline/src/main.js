...
class App {
    ...
    playTypingGame() {
        ...
        if(!this.typingGame_data) {
            // httpではなくfileのプロトコルによりfetchが使えない状況なので、
            // jsonではなくjsの変数に保存する方法でデータを取得
            ...
            // typingGame_contents変数が存在し、中身があるかチェックする
            // 無ければ通知し、falseを返す
            if(typeof typingGame_contents === 'undefined' || typingGame_contents === null) {
                alert('問題データが見つかりません');

                return false;
            }

            this.typingGame_data = { questionContents: typingGame_contents };
